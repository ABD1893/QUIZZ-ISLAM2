...
.tnum{text-align:center;font-size:22px;font-weight:800;margin:.2rem 0 .4rem;}
.tpause{text-align:center;font-size:12px;color:#bbb;margin-bottom:.4rem;font-style:italic;}
...
.qhid{font-size:13px;font-weight:600;color:#ccc;font-style:italic;padding:.4rem 0;}
...
.cfbox{background:#faf9f6;border-radius:10px;border:1.5px solid #e0ddd6;padding:9px 11px;text-align:center;}
.cfbox p{font-size:12px;color:#444;margin-bottom:7px;font-weight:600;}
.cfrow{display:flex;gap:7px;justify-content:center;}
.bconf{padding:6px 16px;border-radius:8px;border:none;background:var(--dk);color:#fff;font-size:12px;font-weight:700;cursor:pointer;}
.bcanc{padding:6px 11px;border-radius:8px;border:1.5px solid #ccc;background:#fff;font-size:12px;cursor:pointer;color:#555;font-weight:600;}
...
.ob.pend{background:#eef3ff;border-color:#4f8ef7;border-width:2px;}
...
.cpend{background:#fffbe6;border:1.5px solid var(--g);border-radius:10px;padding:7px 10px;font-size:13px;font-weight:600;color:var(--dk);margin-bottom:6px;}
...
.hcr{font-size:12px;color:#0a3a20;padding:7px 11px;background:linear-gradient(135deg,#f0faf5,#e8f5f0);border-radius:9px;border:1px solid #b0e0c8;margin-bottom:.5rem;font-weight:600;}
.hprf{font-size:11px;color:var(--g);background:#fffbe6;padding:5px 9px;border-radius:8px;border:1px solid #f0d080;margin-bottom:.5rem;font-style:italic;line-height:1.5;}
...
.ft{font-size:24px;font-weight:800;color:var(--dk);margin-bottom:.3rem;}
.ft b{color:var(--g);}
.podium-simple{display:flex;flex-direction:column;gap:8px;margin:1.2rem 0;}
.pod-row{display:flex;align-items:center;gap:12px;padding:14px 16px;border-radius:13px;background:#faf9f6;border:1px solid #eee;}
.pod-row.first{background:linear-gradient(135deg,#fffbe6,#fff3cc);border:2px solid var(--g);}
.pod-rank{font-size:24px;min-width:36px;text-align:center;}
.pod-name{font-size:14px;font-weight:800;flex:1;text-align:left;}
.pod-score{font-size:15px;font-weight:800;color:#555;}
.pod-rest{display:flex;flex-direction:column;gap:6px;margin-top:.5rem;}
.pod-rest-row{display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-radius:9px;background:#faf9f6;border:1px solid #eee;font-size:13px;}
...
let prevSc=[],tv=60,td=false,tStarted=false;
let hEditTi=-1;
...
function onM(evt){
  ...
  if(m.type==="state"){
    ...
    st=m;tv=m.timerVal;td=m.timerDone;tStarted=m.timerStarted;
...
function rEntry(app){app.innerHTML=\`<div class="card"><div class="logo">Quiz <b>Coran</b></div><div class="gl"></div><p class="sub">51 questions · Sciences islamiques</p>
  <div class="eg">
...
function rWait(app){
  ...
  const slots=teams.map(t=>\`<div class="ts\${t.ready?" rdy":""}"><span class="tn" style="color:\${t.color}">\${t.name}</span>
    \${role==="host"?\`<span class="tc">\${t.code}</span>\`:""}
    <span class="tag \${t.ready?"tok":"twt"}">\${t.ready?"Prête ✓":"En attente..."}</span></div>\`).join("");
  const hSec=role==="host"?\`<div style="margin-top:1rem;padding-top:1rem;border-top:1px solid #eee">
    ...
    <table class="ct"><tr><th>Équipe</th><th>Code</th><th>Membres</th><th>Statut</th></tr>
      \${teams.map(t=>\`<tr><td style="color:\${t.color};font-weight:800">\${t.name}</td><td><span class="cpill">\${t.code}</span></td>
        ...
        <td>\${t.ready?'<span class="tag tok">✓</span>':'<span class="tag twt">Attente</span>'}</td></tr>\`).join("")}
    </table>
...
function rGame(app){
  ...
  const scBar=\`<div class="sb" style="grid-template-columns:repeat(\${teams.length},1fr)">\`+
    teams.map((t,ti)=>\`<div class="sc" data-ti="\${ti}"><div class="scn" style="color:\${t.color}">\${t.name}</div>
      <div class="scv">\${t.score!==null&&t.score!==undefined?t.score:"•••"}</div>
      <div class="scp" style="width:\${t.score?((t.score/mx)*100):0}%;background:\${t.color}"></div></div>\`).join("")+\`</div>\`;

  const timerHTML=!tStarted
    ?\`<div class="tbg"><div class="tbar" id="tBar" style="width:100%;background:#22b87a"></div></div><div class="tnum" id="tNum" style="color:#22b87a">60s</div><div class="tpause">⏸ En attente d'affichage de la question...</div>\`
    :\`<div class="tbg"><div class="tbar\${tv<=10&&tv>0?" pulse":""}" id="tBar" style="width:\${pct}%;background:\${tc}"></div></div><div class="tnum" id="tNum" style="color:\${tc}">\${tv}s</div>\`;

  // Question: host always sees it, participants see it only when visible
  const qText=(role==="host"||st.questionVisible)&&q.q?\`<div class="qt">\${q.q}</div>\`:\`<div class="qhid">⏳ Question en attente d'affichage...</div>\`;
  const qBox=\`<div class="qb"><div class="qn">Question \${st.current+1} · N°\${st.qid}\${isFun?" · 🎉 BONUS":""}</div>\${qText}</div>\`;

  const cols=Math.min(teams.length,3);
  const cards=\`<div class="tg" style="grid-template-columns:repeat(\${cols},minmax(0,1fr))">\`+teams.map((t,ti)=>rCard(t,ti,q,st.revealed,isFun)).join("")+\`</div>\`;

  const proof=st.proofRevealed&&q.proof?\`<div class="pb"><strong>📖 Preuve :</strong> \${q.proof}</div>\`:"";
  ...
  app.innerHTML=\`<div class="gh"><h1>Quiz <b>Coran</b></h1><div class="pr">Q\${st.current+1}/\${st.total} · N°\${st.qid}</div></div>
    \${scBar}\${timerHTML}\${qBox}\${isFun?\`<div class="fb">🎉 Question bonus</div>\`:""}\${proof}\${cards}\${nextBtn}\${hHTML}\`;}
...
function rCard(t,ti,q,revealed,isFun){
  ...
  if(!isMe){
    // Other teams: status only
    let status="En cours...";
    if(t.hasAnswer){
      if(revealed){
        if(t.mode==="cash")status=t.validated===true?"✓ Correct":"✗ Incorrect";
        else if(t.mode==="square")status=t.answer===q.realAns||t.answer===q.ans?"✓ Correct":"✗ Incorrect";
        else if(t.mode==="duo"){const duo=t.duoOpts||[];const ch=typeof t.answer==="number"?duo[t.answer]:t.answer;status=ch===q.opts[q.ans>=0?q.ans:q.realAns]?"✓ Correct":"✗ Incorrect";}
      }else status="Réponse envoyée ✓";
    }else if(t.pending)status="Choix du mode en cours...";
    ...
    if(revealed&&t.mode&&t.hasAnswer){
      ...
      const realAns=q.realAns>=0?q.realAns:(q.ans>=0?q.ans:0);
      if(t.mode==="square"&&t.answer!==null){
        inner+=q.opts.map((o,oi)=>{let c="ob";if(isFun||oi===realAns)c+=" correct";else if(oi===t.answer)c+=" wrong";return\`<button class="\${c}" disabled>\${L[oi]}. \${o}</button>\`;}).join("");
      }else if(t.mode==="duo"&&t.answer!==null){
        const duo=t.duoOpts||[];
        inner+=duo.map((o,oi)=>{let c="ob";if(isFun||o===q.opts[realAns])c+=" correct";else if(oi===t.answer)c+=" wrong";return\`<button class="\${c}" disabled>\${L[oi]}. \${o}</button>\`;}).join("");
      }else if(t.mode==="cash"){
    ...
    return\`<div class="tc2" style="border-top-color:\${t.color}"><div class="th2"><span class="tnm" style="color:\${t.color}">\${t.name}</span><span class="tpt">\${t.score!==null&&t.score!==undefined?t.score+" pts":"•••"}</span></div>\${inner}</div>\`;
  }

  // My team — full interaction
  if(!t.mode&&!t.pending){
    if(!q.opts||!q.opts.length){
      inner=\`<div style="font-size:12px;color:#bbb;text-align:center;font-style:italic;padding:6px">Chargement...</div>\`;
    }else if(isFun){
    ...
    const lbl2={cash:"Cash — 5 pts",square:isFun?"Répondre":"Carré — 3 pts",duo:"Duo — 1 pt"};
    inner=\`<div class="cfbox"><p>Confirmer <strong>\${lbl2[t.pending]}</strong> ?</p>
      <div class="cfrow"><button class="bcanc" onclick="snd({type:'team-cancel-mode'})">Changer</button>
      <button class="bconf" onclick="snd({type:'team-confirm-mode'})">Confirmer ✓</button></div></div>\`;
  }else if(t.mode){
    ...
    if(!t.hasAnswer){
      if(t.mode==="cash"){
        ...
        if(hasPend){
          inner=lbl+\`<div class="cpend">« \${t.pendingAnswer} »</div>
            <div class="cfbox"><p>Confirmer cette réponse ?</p>
              <div class="cfrow"><button class="bcanc" onclick="snd({type:'team-cancel-answer'})">Modifier</button>
              <button class="bconf" onclick="snd({type:'team-confirm-answer',cashText:'\${String(t.pendingAnswer||"").replace(/'/g,"\\\\'")}'})" >Confirmer ✓</button></div></div>\`;
        }else{
          inner=lbl+\`<input class="ci" placeholder="Votre réponse..." oninput="cashVal=this.value" onkeydown="if(event.key==='Enter'&&cashVal.trim())snd({type:'team-pending-answer',answer:cashVal.trim()})"/>
            <button class="btn bg" style="padding:8px;font-size:12px;margin-bottom:0" onclick="if(cashVal.trim())snd({type:'team-pending-answer',answer:cashVal.trim()})">Valider →</button>\`;
        }
      }else if(t.mode==="square"){
        const pend=t.pendingAnswer;
        if(pend!==null&&pend!==undefined){
          inner=lbl+q.opts.map((o,oi)=>\`<button class="ob\${oi===pend?" pend":""}" disabled>\${L[oi]}. \${o}</button>\`).join("")+
            \`<div class="cfbox" style="margin-top:6px"><p>Confirmer \${L[pend]}. \${q.opts[pend]} ?</p>
              <div class="cfrow"><button class="bcanc" onclick="snd({type:'team-cancel-answer'})">Changer</button>
              <button class="bconf" onclick="snd({type:'team-confirm-answer'})">Confirmer ✓</button></div></div>\`;
        }else{
          inner=lbl+q.opts.map((o,oi)=>\`<button class="ob" onclick="snd({type:'team-pending-answer',answer:\${oi}})">\${L[oi]}. \${o}</button>\`).join("");
        }
      }else if(t.mode==="duo"){
        const duo=t.duoOpts||[];
        const pend=t.pendingAnswer;
        if(pend!==null&&pend!==undefined){
          inner=lbl+duo.map((o,oi)=>\`<button class="ob\${oi===pend?" pend":""}" disabled>\${L[oi]}. \${o}</button>\`).join("")+
            \`<div class="cfbox" style="margin-top:6px"><p>Confirmer \${L[pend]}. \${duo[pend]} ?</p>
              <div class="cfrow"><button class="bcanc" onclick="snd({type:'team-cancel-answer'})">Changer</button>
              <button class="bconf" onclick="snd({type:'team-confirm-answer'})">Confirmer ✓</button></div></div>\`;
        }else{
          inner=lbl+duo.map((o,oi)=>\`<button class="ob" onclick="snd({type:'team-pending-answer',answer:\${oi}})">\${L[oi]}. \${o}</button>\`).join("");
        }
      }
    }else{
      // answered
      ...
      if(t.mode==="cash"){
        ...
        const btns=q.opts.map((o,oi)=>{let c="ob";if(revealed){if(isFun||oi===realAns)c+=" correct";else if(oi===t.answer)c+=" wrong";}else if(oi===t.answer)c+=" sel";return\`<button class="\${c}" disabled>\${L[oi]}. \${o}</button>\`;}).join("");
        const tg=revealed?(isFun?\`<span class="tag tok">🎉</span>\`:(t.answer===realAns?\`<span class="tag tok">Correct +3 🎯</span>\`:\`<span class="tag tko">Incorrect ✗</span>\`)):\`<span class="tag tan">Répondu ✓</span>\`;
        inner=lbl+btns+tg;
      }else if(t.mode==="duo"){
        const duo=t.duoOpts||[];
        const btns=duo.map((o,oi)=>{let c="ob";if(revealed){if(isFun||o===q.opts[realAns])c+=" correct";else if(oi===t.answer)c+=" wrong";}else if(oi===t.answer)c+=" sel";return\`<button class="\${c}" disabled>\${L[oi]}. \${o}</button>\`;}).join("");
        const ch=typeof t.answer==="number"&&duo[t.answer]?duo[t.answer]:t.answer;
        const ok2=ch===q.opts[realAns];
        const tg=revealed?(ok2?\`<span class="tag tok">Correct +1 🎯</span>\`:\`<span class="tag tko">Incorrect ✗</span>\`):\`<span class="tag tan">Répondu ✓</span>\`;
        inner=lbl+btns+tg;
      }
    }
  }
...
function rHP(teams,q,st2,isFun){
  // Use st2.timerDone OR global td for canReveal
  const allAns=teams.every(t=>t.hasAnswer);
  const canRev=(st2.timerDone||td||allAns)&&!st2.revealed;
  // Use realAns for host display — fixes the "toutes" bug
  const realAns=q.realAns>=0?q.realAns:q.ans;
  const correctLabel=isFun?"toutes (question bonus)":q.opts&&realAns>=0?q.opts[realAns]:"—";
  ...
  const rows=teams.map((t,ti)=>{
    ...
    // Show pendingAnswer too
    if(t.hasAnswer){
      if(t.mode==="cash"){
        if(hEditTi===ti){
          ans=\`<input class="hi" id="hed\${ti}" value="\${t.answer||""}" onkeydown="if(event.key==='Enter')sC(\${ti})"/>\`;
          act=\`<button class="bgsm" onclick="sC(\${ti})">OK</button>\`;
        }else{
          ...
          if(t.validated===null||t.validated===undefined){
            ...
            act=(t.validated?\`<span class="tag tok" style="font-size:10px">Validé</span>\`:\`<span class="tag tko" style="font-size:10px">Invalidé</span>\`)+
              \`<button class="bgsm" onclick="snd({type:'host-recheck',teamIdx:\${ti}})">Revoir</button>\`;
          }
        }
      }else if(t.mode==="square"){
      ...
      // Show pending (not yet confirmed) in orange
      const pStr=t.mode==="cash"?"« "+t.pendingAnswer+" »":(q.opts&&t.pendingAnswer<q.opts.length?L[t.pendingAnswer]+". "+q.opts[t.pendingAnswer]:"...");
      ans=\`<span class="han" style="color:#f7934f">⏳ \${pStr} (confirmation en cours)</span>\`;
    }
  ...
  const showQBtn=!st2.questionVisible
    ?\`<button class="btn bg" style="max-width:260px;padding:9px;margin-bottom:.4rem" onclick="snd({type:'host-show-question'})">👁 Afficher la question</button>\`
    :\`<span class="tag tok" style="display:inline-block;margin-bottom:.4rem">Question visible ✓</span>\`;

  const scToggle=\`<button class="bsm" onclick="snd({type:'host-toggle-scores'})" style="margin-bottom:.5rem">\${st2.scoresVisible?"🔒 Masquer scores":"👁 Afficher scores"}</button>\`;

  const revBtns=!st2.revealed
    ?\`<button class="btn bgo" style="max-width:260px" \${canRev?"":\`disabled\`} onclick="snd({type:'host-reveal-answer'})">🎯 Révéler la bonne réponse</button>
       \${!canRev?\`<div class="rh">Attente équipes ou fin du timer...</div>\`:""}\`
    :(!st2.proofRevealed
      ?\`<button class="btn bgo" style="max-width:260px" onclick="snd({type:'host-reveal-proof'})">📖 Révéler la preuve</button>\`
      :\`<span class="tag tok">✓ Preuve révélée</span>\`);

  return\`<div class="hp">
    <h3>🎙️ Panneau hôte</h3>
    <div class="hcr"><strong>Bonne réponse :</strong> \${correctLabel}</div>
    <div class="hprf"><strong>Preuve :</strong> \${q.fullProof||"—"}</div>
    \${rows}
    ...
    <div class="ra">
      <div style="margin-bottom:.4rem">\${scToggle}</div>
      ...
      \${showQBtn}<div style="margin-top:.4rem">\${revBtns}</div>
...
function rEnd(app){
  ...
  const podHTML=\`<div class="podium-simple">
    \${top3.map((t,i)=>\`<div class="pod-row\${i===0?" first":""}">
      <span class="pod-rank">\${medals[i]}</span>
      <span class="pod-name" style="color:\${t.color}">\${t.name}</span>
      <span class="pod-score">\${t.score||0} pts</span>
    </div>\`).join("")}
  </div>\`;

  const restHTML=rest.length?\`<div class="pod-rest">\${rest.map((t,i)=>\`<div class="pod-rest-row">
    <span style="font-weight:700;color:\${t.color}">\${t.name}</span>
    <span style="color:#888;font-weight:600">\${t.score||0} pts</span>
  </div>\`).join("")}</div>\`:"";
...
function fresh() {
  return {
    phase:"setup", teams:[], teamCodes:{}, current:0,
    qState:[], revealed:false, proofRevealed:false,
    questionVisible:false, scoresVisible:true,
    timerVal:60, timerDone:false, timerStarted:false
  };
}
...
function startQ() {
  ...
  game.revealed=false; game.proofRevealed=false;
  game.questionVisible=false;
  game.timerVal=60; game.timerDone=false; game.timerStarted=false;
  // Timer starts only when host shows question
}

let timerInt=null;
function startTimer(){
  stopTimer(); game.timerStarted=true;
  timerInt=setInterval(()=>{
    ...
    if(!game.timerDone&&game.questionVisible&&game.qState.every(s=>s.mode&&s.answer!==null)){
...
function addTime(s){game.timerVal+=s;game.timerDone=false;if(!timerInt&&game.timerStarted)startTimer();broadcast({type:"timer",val:game.timerVal,done:false});}
...
function buildState(forHost){
  ...
  const teams=game.teams.map((t,ti)=>{
    ...
    if(forHost){
      return {...t,mode:s.mode,pending:s.pending,answer:s.answer,
        pendingAnswer:s.pendingAnswer,hasAnswer:s.answer!==null,
  ...
  return {
    type:"state",forHost,phase:game.phase,teams,
    current:game.current,total:FIXED_ORDER.length,qid,
    question:forHost?questionForHost:questionForPub,
  ...
  ws.on("message",raw=>{
    ...
    switch(msg.type){
      ...
      case "host-reveal-proof":
        if(info.role!=="host")break;game.proofRevealed=true;broadcastState();break;
      case "host-next":
        if(info.role!=="host")break;
        game.current++;
      ...
      case "host-toggle-scores":
        if(info.role!=="host")break;game.scoresVisible=!game.scoresVisible;broadcastState();break;
      ...
      case "host-recheck":
        if(info.role!=="host"||!game.qState[msg.teamIdx])break;
        game.qState[msg.teamIdx].validated=null;broadcastState();break;
      case "host-bonus":
      ...
      case "team-auth":
        ...
        }else sendTo(ws,{type:"team-auth-fail",reason:idx!==undefined?"already-used":"invalid"});
        break;
      case "team-set-info":
      ...
      case "team-select-mode":
        ...
        game.qState[info.teamIdx].pending=msg.mode;broadcastState();break;
      ...
      case "team-pending-answer":
        ...
        game.qState[info.teamIdx].pendingAnswer=msg.answer;broadcastState();break;
      ...
      case "team-cancel-answer":
        if(info.role!=="team"||!game.qState[info.teamIdx]||game.qState[info.teamIdx].answer!==null)break;
        game.qState[info.teamIdx].pendingAnswer=null;broadcastState();break;
    }
  });
